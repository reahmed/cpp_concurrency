...
public:
  ...
  void wait()
  {
    ...
    if (q_wait.size() == 0)
      ...
      q_wait.push(q_wait.back() + 1);
    
    int my_wait_number = q_wait.back();
    ...
    while (count == 0 or (my_wait_number != q_wait.front()))
      cv.wait(lock);
  ...
  int count = N;
  std::queue<int> q_wait;

  int num_waiting(){ return q_wait.size(); }
...
int main()
{
  ...
  while (std::getline(std::cin, line))
  {
    ...
    else if (line == "w")
    {
      ...
      ths.push_back(new std::thread(&SemaphoreReleaseFIFO::wait, &sm));
    }
    else if (line == "s")
    {
      // spawn thread that calls signal on semaphore
      ths.push_back(new std::thread(&SemaphoreReleaseFIFO::signal, &sm));
    }
    else
    {
      // ignore line
