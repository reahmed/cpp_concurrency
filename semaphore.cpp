...
public:
  ...
  void wait()
  {
    ...
    // check that count != 0 before continuing to avoid
    // spurious wakeups in pthreads.
  ...
  void signal()
  {
    ...
    // if count was not zero before, no need to notify
    // even if it was zero, not guaranteed that there is
    // a thread waiting.
...
int main()
{
  ...
  while (std::getline(std::cin, line))
  {
    ...
    else if (line == "w")
    {
      ...
      ths.push_back(new std::thread(&Semaphore::wait, &sm));
    }
    else if (line == "s")
    {
      // spawn thread that calls signal on semaphore
      ths.push_back(new std::thread(&Semaphore::signal, &sm));
    }
    else
    {
      // ignore line
